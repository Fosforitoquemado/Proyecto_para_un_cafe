...
func esperar_y_titilar():
	while true:
		...
		await get_tree().create_timer(randf_range(2.0, 15.0)).timeout
		#EL FALLO: Hace una ráfaga de parpadeos rápidos (entre 3 y 7 chispazos)
		var chispazos = randi_range(2, 5)
